...
int main(void) {
	...
	while(1) {
		...
		request = (Trequest *)malloc(sizeof(Trequest));
		
		r = read(socketToClient, buf, 5);
		request->fd = socketToClient;
		sscanf (buf, "%c %d", &request->chr, &request->time);
		
		
		// Create a new thread to serve the new client
		error = pthread_create(&thread_id, NULL, serveClient, request);
		if (error) 
...
void *serveClient(void *ptr) {
	...
	//if (r > 0) {
		...
		for (i = 0; i < 10; i++) {
			if (write(request->fd, &request->chr, 1) == -1)
